 ...
 * @author IronSid
 * @version 1.0
 * @since 2023-03-05 00:04:30 
 */
public class MaximumProfitOfOperatingACentennialWheelSolution {
static
...
class Solution {
    public int minOperationsMaxProfit(int[] customers, int boardingCost, int runningCost) {
        ...
        for (int i = 0; i < customers.length; i++) {
            remain += customers[i];
            profit += Math.min(4, remain) * boardingCost - runningCost;
            if (profit > max) {
                max = profit;
                ans = i + 1;
            }
            remain = Math.max(0, remain - 4);
        }
        if (remain >= 4 && boardingCost * 4 > runningCost) {
            int r = remain / 4;
            profit += r * 4 * boardingCost - r * runningCost;
            if (profit > max) {
                max = profit;
                ans = customers.length + r;
            }
        }
        remain = remain % 4;
        if (boardingCost * remain > runningCost) {
