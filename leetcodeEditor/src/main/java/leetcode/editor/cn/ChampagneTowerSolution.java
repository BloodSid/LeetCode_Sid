...
class Solution {
    ...
    public double champagneTower(int poured, int x, int y) {
        if (x == 0) return poured > 0 ? 1.0 : 0.0;
        // dp[i][j] 表示从 (i,j) 的杯子流出的酒
        dp = new double[x][x];
        for (double[] o : dp) {
            Arrays.fill(o, -1);
        }
        dp[0][0] = poured - 1;
        return Math.min(1.0, 0.5 * (f(x - 1, y) + f(x - 1, y - 1)));
    }

    double f(int x, int y) {
        if (y < 0 || y > x) return 0;
        if (dp[x][y] < 0) {
            dp[x][y] = Math.max(0.0, 0.5 * (f(x - 1, y) + f(x - 1, y - 1)) - 1.0);
        }
        return dp[x][y];
    }
}
