 ...
 * @author IronSid
 * @version 1.0
 * @since 2023-01-02 00:23:53 
 */
public class NumberOfOrdersInTheBacklogSolution {
static
//leetcode submit region begin(Prohibit modification and deletion)
class Solution {
    public int getNumberOfBacklogOrders(int[][] orders) {
        // {price, amount, id} 按 price 减排序
        PriorityQueue<int[]> buy = new PriorityQueue<>((o1, o2) -> o1[0] != o2[0] ? o2[0] - o1[0] : o1[2] - o2[2]);
        // 按 price 增排序
        PriorityQueue<int[]> sell = new PriorityQueue<>((o1, o2) -> o1[0] != o2[0] ? o1[0] - o2[0] : o1[2] - o2[2]);
        int id = 0;
        for (int[] order : orders) {
            if (order[2] == 0) {
                // 采购
                while (!sell.isEmpty() && order[1] > 0 && order[0] >= sell.peek()[0]) {
                    int[] first = sell.peek();
                    int match = Math.min(order[1], first[1]);
                    first[1] -= match;
                    if (first[1] == 0) sell.poll();
                    order[1] -= match;
                }
                if (order[1] > 0) {
                    order[2] = id;
                ...
                // 卖出
                while (!buy.isEmpty() && order[1] > 0 && order[0] <= buy.peek()[0]) {
                    int[] first = buy.peek();
                    int match = Math.min(order[1], first[1]);
                    first[1] -= match;
                    if (first[1] == 0) buy.poll();
                    order[1] -= match;
                }
                if (order[1] > 0) {
                    order[2] = id;
