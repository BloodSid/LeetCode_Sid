 ...
 * @author IronSid
 * @version 1.0
 * @since 2023-03-22 02:55:25 
 */
public class BestTeamWithNoConflictsSolution {
static
...
class Solution {
    public int bestTeamScore(int[] scores, int[] ages) {
        ...
        int ans = 0;
        int[] dp = new int[n];
        // 求年龄的最长递增子序列，这里的长度定义为子序列的 score 之和
        for (int i = 0; i < n; i++) {
            dp[i] = scores[idx[i]];
            for (int j = 0; j < i; j++) {
                if (ages[idx[j]] <= ages[idx[i]]) dp[i] = Math.max(dp[i], dp[j] + scores[idx[i]]);
            }
