...
class Solution {
    public String findLexSmallestString(String s, int a, int b) {
        ...
        // b 是奇数则 s 的奇偶位置都能累加， b 是偶数则只能累加奇数位置
        int[] d = digit.clone();
        // 枚举累加
        for (int i = 0; i < 10; i++) {
            // 累加奇数位置
            for (int t = 1; t < d.length; t += 2) {
                d[t] = (digit[t] + i * a) % 10;
            }
            if (i != 0 && d[1] == digit[1]) break;
            for (int j = 0; j < 10; j++) {
                // 累加偶数位置
                for (int t = 0; t < d.length; t += 2) {
                    d[t] = (digit[t] + j * a) % 10;
                }
                if (j != 0 && d[0] == digit[0]) break;
                ...
                for (int k = 0; k < s.length(); k++) {
                    int rotate = k * b % s.length();
                    if (k != 0 && rotate == 0) break;
                    String t = sb.substring(rotate) + sb.substring(0, rotate);
                    if (t.compareTo(s) < 0) {
                        s = t;
                    }
                }
