 ...
 * @author IronSid
 * @version 1.0
 * @since 2024-03-28 11:15:14 
 */
public class FirstDayWhereYouHaveBeenInAllTheRoomsSolution {
static
...
class Solution {
    ...
    public int firstDayBeenInAllRooms(int[] nextVisit) {
        ...
        // dp0(i) 表示到达i 偶数次的日期，dp1(i) 表示到达 dp(0) 奇数次
        long[][] dp = new long[2][n];
        // 第0天第1次到位置0，第1天第2次到位置1
        dp[0][0] = 1;
        for (int i = 1; i < n; i++) {
            dp[1][i] = (dp[0][i - 1] + 1) % M;
            dp[0][i] = (dp[1][i] + (dp[1][i] - dp[1][nextVisit[i]]) + 1 + M) % M;
        }
        return (int) dp[1][n - 1];
    }
}
