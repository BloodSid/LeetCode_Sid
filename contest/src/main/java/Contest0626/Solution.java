...
public class Solution {
    ...
    public int minimumScore(int[] nums, int[][] edges) {
        ...
        dfs(0, parent, xorSum, map, visited);
    ...
    private void dfs(int node, int[] parent, int[] xorSum, List[] map, boolean[] visited) {
        visited[node] = true;
        int xor = 0;
        for (Object o : map[node]) {
            ...
            parent[chlid] = node;
            dfs(chlid, parent, xorSum, map, visited);
            xor ^= xorSum[chlid];
        }
        xorSum[node] = xor;
    }
    ...
    boolean isSub(int node1, int node2, int[] parent) {
        // 根节点不是任何节点的子树
        if (node1 == 0) return false;
        if (parent[node1] == node2) return true;
        return isSub(parent[node1], node2, parent);
    }
